...
export default function SideDrawer() {
  ...
  const isSmallScreen = useBreakpointValue({ sm: false, md: true ,base:false,lg:true });
  const logoutHandler = () => {
  ...
  const handleSearch = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(
        `http://localhost:8888/api/user?search=${search}`,
        config
      );
      ...
      >
        <Tooltip label="Search Users to Chat" hasArrow placement="bottom-end">
          <Button variant="ghost" onClick={onOpen}>
            <ImSearch style={{ fontSize: "23px" }} />
            {isSmallScreen && <Text px="4">Search User</Text>}
          </Button>
        </Tooltip>
        ...
        <div>
          ...
          <Menu>
            <MenuButton
              as={Button}
              rightIcon={<BsThreeDotsVertical style={{ fontSize: "23px" }} />}
            >
              <Avatar
                size="sm"
                name={user.name}
